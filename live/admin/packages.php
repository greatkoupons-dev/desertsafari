...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  ...
  if ($action === 'save') {
    ...
    if ($id) {
      q("UPDATE packages SET name=?, short_desc=?, price_badge=?, image_url=?, is_active=?, sort_order=? WHERE id=?",
        [$name,$short_desc,$price_badge,$image_url,$is_active,$sort_order,$id]);
    } else {
      q("INSERT INTO packages(name, short_desc, price_badge, image_url, is_active, sort_order, created_at) VALUES(?,?,?,?,?,?,?)",
  ...
  if ($action === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id) q("DELETE FROM packages WHERE id=?", [$id]);
  }
        ...
        <div class="card">
          ...
          <form method="post">
            ...
            <div style="margin-top:10px">
              ...
              <textarea class="input" name="short_desc"><?= e($editing['short_desc'] ?? '') ?></textarea>
            </div>
              ...
              <tbody>
                <?php foreach($list as $p): ?>
                  <tr>
                    ...
                    <td class="row">
                      <a class="btn" href="<?= e(url('admin/packages.php?edit='.$p['id'])) ?>">Edit</a>
                      <form method="post" onsubmit="return confirm('Delete package #<?= (int)$p['id'] ?>?')">
                ...
                <?php endforeach; ?>
                <?php if(!$list): ?><tr><td colspan="5" class="small">No packages yet.</td></tr><?php endif; ?>
              </tbody>
            </table>
          </div>
