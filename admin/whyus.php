...
$editing = $editId ? one("SELECT * FROM whyus WHERE id=?", [$editId]) : null;
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  ...
  if ($action === 'save') {
    ...
    if ($title === '') redirect(url('admin/whyus.php'));

    if ($id) {
      q("UPDATE whyus SET title=?, subtitle=?, image_url=?, is_active=?, sort_order=? WHERE id=?",
        [$title,$subtitle,$image_url,$is_active,$sort_order,$id]);
    } else {
      q("INSERT INTO whyus(title, subtitle, image_url, is_active, sort_order, created_at) VALUES(?,?,?,?,?,?)",
  ...
  if ($action === 'delete') {
    $id = (int)($_POST['id'] ?? 0);
    if ($id) q("DELETE FROM whyus WHERE id=?", [$id]);
  }

  redirect(url('admin/whyus.php'));
}

$list = all("SELECT * FROM whyus ORDER BY sort_order ASC, id DESC");
?>
<!doctype html>
<html>
<head>
  ...
  <title>Why Us • DesertSafariGo</title>
...
<body>
  ...
  <div style="padding:18px 0">
    <div class="container">
      <div class="grid">
        <div class="card">
          <div class="h1"><?= $editing ? 'Edit Why Us Item' : 'Add Why Us Item' ?></div>
          <form method="post">
            ...
            <div class="row" style="margin-top:14px">
              ...
              <?php if($editing): ?><a class="btn" href="<?= e(url('admin/whyus.php')) ?>">Cancel</a><?php endif; ?>
            </div>
          </form>
        </div>
              ...
              <tbody>
                <?php foreach($list as $p): ?>
                  <tr>
                    ...
                    <td class="row">
                      <a class="btn" href="<?= e(url('admin/whyus.php?edit='.$p['id'])) ?>">Edit</a>
                ...
                <?php endforeach; ?>
                <?php if(!$list): ?><tr><td colspan="5" class="small">No items yet.</td></tr><?php endif; ?>
              </tbody>
            </table>
          </div>
          <div class="small" style="margin-top:10px">These are shown in the parallax “Why us” section.</div>
        </div>
      </div>
    </div>
  </div>
